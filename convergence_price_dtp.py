...
def setup_japanese_font():
    ...
    if os.path.exists(font_dir):
        ...
        for font_file in font_files:
            ...
            try:
                ...
                print(f"フォントを追加しました: {font_file}")
            except Exception as e:
                print(f"フォント追加エラー: {font_file} - {e}")
    
    # 利用可能な日本語フォントの候補
    if system == "Windows":
        japanese_fonts = ['Meiryo', 'MS Gothic', 'Yu Gothic', 'BIZ UDPGothic', 'NotoSansCJK-Regular', 'Noto Sans CJK JP']
    elif system == "Darwin":  # macOS
        japanese_fonts = ['Hiragino Sans', 'Hiragino Kaku Gothic ProN', 'AppleGothic', 'NotoSansCJK-Regular', 'Noto Sans CJK JP']
    else:  # Linux
        japanese_fonts = ['Noto Sans CJK JP', 'IPAexGothic', 'IPAPGothic', 'DejaVu Sans', 'NotoSansCJK-Regular']
    ...
    # 利用可能なフォントの中から日本語対応フォントを探す
    japanese_keywords = ['Gothic', 'Mincho', 'Sans', 'CJK', 'JP', 'Meiryo', 'Yu']
    for font in available_fonts:
        for keyword in japanese_keywords:
            if keyword in font:
                plt.rcParams['font.family'] = font
                print(f"日本語対応と思われるフォントを設定しました: {font}")
                return font
    
    # フォールバック: デフォルトフォントでマイナス記号の文字化け対策のみ
    plt.rcParams['axes.unicode_minus'] = False
    print("日本語フォントが見つかりません。デフォルトフォントを使用します。")
    print(f"利用可能なフォント（一部）: {available_fonts[:10]}")
    return None
...
def create_main_layout():
    ...
    layout = [
        ...
        [
            ...
            eg.Input("^N225", key="-TICKER-", size=(15, 1)),
            eg.Text("期間:", size=(5, 1)),
...
def main():
    ...
    window = eg.Window("株価収束検出システム", create_main_layout(), 
                      finalize=True, resizable=True, size=(930, 1000))
